...
class Ego4DLiveChatGeneration(EgoExo4D):
    @staticmethod
    def get_narrations(split):
        annos = []
        sources = json.load(open(os.path.join(EgoExo4D.anno_root, f'goalstep_{split}.json')))['videos']
    ...
    def __init__(self, num_queries_each_conversation: int, num_conversations_each_video: int, **kwargs):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained('/data1/Models/Meta-Llama-3-8B-Instruct', use_fast=True)
        self.model = AutoModelForCausalLM.from_pretrained('/data1/Models/Meta-Llama-3-8B-Instructt', torch_dtype='auto', attn_implementation='sdpa')
        self.model.to('cuda')
        self.model.eval()
    ...
    def __call__(self, index):
        ...
        for nt in range(self.num_conversations_each_video):
            ...
            try:
                ...
                os.makedirs(f'{EgoExo4D.anno_root}/livechat/', exist_ok=True)
                json.dump(anno, open(f'{EgoExo4D.anno_root}/livechat/{video_uid}_{index}_{nt}.json', 'w'), indent=4)
            except:
                print('\n---\n' + text + '\n---\n')
