...
def distributed_ffmpeg(*, src_root: str, fps: int = None, resolution: int = None, pad: str = '#000000', mode='bicubic', **kwargs):
    ...
    src_paths = [str(path) for path in pather.rglob('*') if path.is_file() and str(path).endswith('.mp4') and (path.name == 'aria01_214-1.mp4' or path.name.startswith('cam'))]

    dst_root = 'egoexo_data_384ss/takes'
...
def distributed_encode(*, src_root: str, vision_pretrained: str, vision_encode: callable, batch_size: int, embed_mark: str, save_bf16: bool = False, **kwargs):
    ...
    dst_root = f"{src_root}_{vision_pretrained.replace('/', '--')}"
    os.makedirs(dst_root, exist_ok=True)

    pather = pathlib.Path(src_root)
    src_paths = [str(path) for path in pather.rglob('*') if path.is_file() and str(path).endswith('.mp4')]
    
    assigned_files = [file for i, file in enumerate(src_paths) if i % env.num_tasks == env.global_rank]
    print(f"Task {env.global_rank} processing {len(assigned_files)} files")
    
    for i, file in tqdm.tqdm(enumerate(src_paths), desc=f'{src_root} -> {dst_root}'):
        if i % env.num_tasks != env.global_rank:
            continue
