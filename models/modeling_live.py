...
# from .live_llama.modeling_mod_llama import MoDLlamaForCausalLM
...
class LiveMixin(AutoModelForCausalLM):
    ...
    def visual_embed(self, frames: torch.Tensor, frames1: torch.Tensor): # moe
        ...
        frames1 = self.connector1(frames1)
        
        weights = self.router1(frames[:, 0, :]).view(-1, 10, 2).softmax(dim=-1) # best
        frames_fusion = frames * weights[:, :, 0, None] + frames1 * weights[:, :, 1, None] # best
        
        return frames_fusion.view(-1, frames_fusion.shape[-1])

    def joint_embed(
        self,
        input_ids: torch.Tensor = None,
        frames: torch.Tensor = None,
        frames1: torch.Tensor = None,        
    ):
        
        if frames is None:
            v_mask = input_ids == self.config.v_placeholder_id
            return self.get_input_embeddings()(input_ids), v_mask.to(torch.int)
        if input_ids is None:
            return self.visual_embed(frames,frames1)
        inputs_embeds = self.get_input_embeddings()(input_ids.clamp(max=self.vocab_size-1))
        v_mask = input_ids == self.config.v_placeholder_id

        if v_mask.any():
            inputs_embeds[v_mask] = self.visual_embed(frames,frames1)  # moe
        
        return inputs_embeds, v_mask.to(torch.int)
    ...
    def stream_evaluate(
        ...
        **kwargs
        ...
        outputs = self.forward(input_ids=input_ids, frames=frames, frames1=frames1,return_dict=True, use_cache=True)
                ...
                if turn_stream_masked_pred_mask.any():
                    ...
                    if r == num_turns - 1: # no future frame. we assume the model should receive a signal when streaming ends (e.g. close button).
                        ...
                        if to_append_num_frames == 0:
                            ...
                            to_append_frames1 = frames1[past_num_frames+turn_num_frames:past_num_frames+turn_num_frames+to_append_num_frames]
