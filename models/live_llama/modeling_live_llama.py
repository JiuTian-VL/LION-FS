...
from .mod_llama import MoDLlamaForCausalLM
from ..modeling_live import build_live, LiveMixin

logger = logging.get_logger(__name__)

class LiveLlamaForCausalLM(MoDLlamaForCausalLM, LiveMixin):
    ...
    def __init__(self, config: LiveLlamaConfig):
        ...
        self.connector1 = torch.nn.Sequential(            
            torch.nn.Linear(config.vision_hidden_size1, config.hidden_size, bias=True),
            GELUActivation(config.hidden_size),
            torch.nn.Linear(config.hidden_size, config.hidden_size, bias=True),
        )
        ...
        self.router1 = nn.Sequential(
            ...
            nn.GELU(),
            torch.nn.Linear(config.hidden_size, 20),
        )
    ...
    def forward(
        ...
        frames1: torch.FloatTensor = None,
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            inputs_embeds, v_mask = self.joint_embed(input_ids, frames, frames1)  #moe
    ...
    def generate_after_embed(self, input_ids, frames, frames1, **kwargs):
        return super().generate(inputs_embeds=self.joint_embed(input_ids, frames, frames1), **kwargs)
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, cache_position=None, position_ids=None, use_cache=True, **kwargs):
        results = super().prepare_inputs_for_generation(input_ids, past_key_values, attention_mask, inputs_embeds, cache_position, position_ids, use_cache, **kwargs)
        results["frames"] = kwargs.get("frames", None)
        results["frames1"] = kwargs.get("frames1", None)
        return results
